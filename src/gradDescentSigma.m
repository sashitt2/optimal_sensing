function [xs, sigma, A, L, objList] = gradDescentSigma(xs0,sigma0,A0,L0,xh,q,Q,R,update,maxIter,step_init, sigmaMax)
...
valueOld = getObjInfinite(A, q0, q, Q, R, C, L);
...
while( abs(relError) > tol && iterCount < maxIter)
    ...
    % update xs, sigma and C
    [~, dCdxs, dCdsigma] = getCfromSensorlocwidth(xs, xh, sigma);
    if update(1)        
        dgdxs = sum(conj(dCdxs).*dgdC, 2);
        xs = xs - step*real(dgdxs);
    end
    if update(2)        
        dgdsigma = sum(conj(dCdsigma).*dgdC, 2);
        sigma = sigma - step*real(dgdsigma);
        sigma = min(abs(sigma), sigmaMax);
    end        
    %C = exp(-(xh-xs).^2/2/sigma^2)'*M;
    [C, ~] = getCfromSensorlocwidth(xs, xh, sigma);
    ...
    valueNew = getObjInfinite(A, q0, q, Q, R, C, L);    
